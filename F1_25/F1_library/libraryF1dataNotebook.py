...
def obtain_data_tyres(dataset,compound,duration):
    ...
    """
    Function: obtain_drivers_colours
    Description: Obtain the information of the colours of the drivers adding a #
    Parameters:
    drivers: Drivers to obtain his colours
    """
def obtain_drivers_colours(drivers):
    global drivers_colours
    dict_drivers = drivers.set_index('name_acronym').to_dict()['team_colour']
    # Update the dict in ordedr to add #
    for driver in dict_drivers:
        dict_drivers[driver] = "#"+dict_drivers[driver]
    drivers_colours = dict_drivers
...
def obtainchart(xvariable,yvariable,dataset):
    sorted_drivers = []
    for dr in dataset.name_acronym:
        sorted_drivers.append(drivers_colours[dr])
    pal = sns.set_palette(sns.color_palette(sorted_drivers))
    plt.figure(figsize=(12, 9))
    plots = sns.barplot(x=xvariable, y=yvariable, data=dataset,palette=pal,hue=xvariable)
...
def build_graph_race(data,title,y_axis,hover_data=None):
    color_compound = {'SOFT':'red','MEDIUM':'yellow','HARD':'grey','INTERMEDIATE':'green','WET':'blue'}
    return px.scatter(data,
                     ...
                     color_discrete_map=color_compound)

def race_pace_graph(data):
    fig = build_graph_race(data,data.full_name.head(1).to_string(index=False)+' laptimes',"lap_duration",['duration_sector_1','duration_sector_2','duration_sector_3'])
    fig.show("png")

def race_pace_divided_per_sectors(data):
    color_compound = {'SOFT':'red','MEDIUM':'yellow','HARD':'grey','INTERMEDIATE':'green','WET':'blue'}

    figures = [
    build_graph_race(data,'Sector 1',"duration_sector_1")
    ,
    build_graph_race(data,'Sector 2',"duration_sector_2")
    ,
    build_graph_race(data,'Sector 3',"duration_sector_3")
    ...
    fig.update_layout(height=1400, width=1650, title_text="Divided per sectors")

    fig.show("png")      
# Source: https://docs.fastf1.dev/gen_modules/examples_gallery/plot_gear_shifts_on_track.html#sphx-glr-gen-modules-examples-gallery-plot-gear-shifts-on-track-py
    """
    Function: show_shifts_poleman
    Description: Obtain the shifts used in the fastest lap of the poleman
    Parameters:
    dataset: Dataset that contains information about the laps of the session
    """
def show_shifts_poleman(dataset):
    laps = dataset.laps.pick_fastest()
    ...
    cbar = plt.colorbar(mappable=lc_comp, label="Number of the Gear",
    ...
    Description: Obtain the telemetry of the poleman compared with the second.
    Parameters:
    dataset: Dataset that contains information about the laps of the session
   session: Fast F1 session
    """
def plot_telemetry(xvariable,yvarible,dataset,session):
    pole = session.laps.pick_driver(dataset.name_acronym[:1].to_string(index=False)).pick_fastest()
    pole_data = pole.get_car_data().add_distance()
    second = session.laps.pick_driver(dataset.name_acronym[1:2].to_string(index=False)).pick_fastest()
    second_data = second.get_car_data().add_distance()

    fig, ax = plt.subplots(figsize=(20,8))
    ax.plot(pole_data[xvariable], pole_data[yvarible], color="red", label=dataset.name_acronym[:1].to_string(index=False))
    ax.plot(second_data[xvariable], second_data[yvarible], color="blue", label=dataset.name_acronym[1:2].to_string(index=False))
    
    ax.set_xlabel(xvariable)
    ax.set_ylabel(yvarible)
    ax.legend()
    plt.show()

    """
    Function: rotate
    Description: Matemathic formaula that gives the rotation of the turn
    Parameters:
    xy: XY angle
...
"""
Function: draw_track
Description: Draw the track
Parameters:
circuit_info: Information of the circuit
pos: Position
title: Name od the circuit
"""
def draw_track(circuit_info, pos,title):
    ...
    plt.axis('off')
    plt.axis('equal')
    plt.plot(rotated_track[:, 0], rotated_track[:, 1],color="orange")
    plt.show()
